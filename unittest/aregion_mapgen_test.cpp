...
#include "aregion.h"
#include "object.h"
#include "mapgen.h"   // Map, B_* biome constants
...
// The same pattern applied to the map-gen leaf helpers (external linkage, no header prototype;
// only referenced within aregion.cpp, transitively through CreateNaturalSurfaceLevel). The
// mapgen suite above *executes* them but asserts nothing about their results; the "mapgen
// helpers" suite below declares them here and pins their behaviour directly. The remaining
// map-gen functions (makeRivers, cleanupIsolatedPlaces, placeVolcanoes, giveNames, ...) take
// file-local types (WaterBody, NameArea) or need a full RNG Map, and stay snapshot territory.
