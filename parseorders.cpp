...
void Game::ProcessDeclareOrder(Faction *f, parser::string_parser& parser, orders_check *checker)
{
	...
	if (fac == -1) f->defaultattitude = att;
	else f->set_attitude(fac, att);
}
...
void Game::ProcessDescribeOrder(Unit *unit, parser::string_parser& parser, orders_check *checker)
{
	...
	if (token == "unit") {
		unit->SetDescribe(new AString(description.get_string()));
		return;
	}
	...
	unit->object->SetDescribe(new AString(description.get_string()));
}
...
void Game::ProcessNameOrder(Unit *unit, parser::string_parser& parser, orders_check *checker)
{
	...
	if (token == "faction") {
		unit->faction->SetName(new AString(name.get_string()));
		return;
	}

	if (token == "unit") {
		unit->SetName(new AString(name.get_string()));
		return;
	}
	...
	if (token == "building" || token == "ship" || token == "object" || token == "structure") {
		...
		unit->object->SetName(new AString(name.get_string()));
		return;
	}
	...
	AString *newname = AString(name.get_string()).getlegal();
	if (!newname) {
...
void Game::ProcessAddressOrder(Unit *u, parser::string_parser& parser, orders_check *checker)
{
	...
	if (checker) return;
	u->faction->address = new AString(token.get_string());
}
