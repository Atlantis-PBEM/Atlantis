...
public:
	...
	int GUARD_REGEN; /* percent chance of regenerating per turn */
	int CITY_GUARD;
	int GUARD_DEPENDS_ON_TAX;
	int GUARD_MONEY;
	int CITY_POP;
	...
	//should the order templates go into template.<x> or report.<x> ?
	int SEPERATE_TEMPLATES;
	...
	int COASTAL_FISH;
	int CITY_MONSTERS_EXIST; // ie Guardsmen
	int WANDERING_MONSTERS_EXIST;
	int LAIR_MONSTERS_EXIST;
	int WEATHER_EXISTS;
	int OPEN_ENDED;
	int NEXUS_EXISTS;
	int CONQUEST_GAME;  // effects genrules and map only
	...
	// Cost of sending goods (-1 if SEND is disabled)
	int SEND_COST;
	...
	//Is the world flat or cylindrical? Atlantis was developed for cylindrical worlds,
	//and this gamedef has only been done briefly for non-icosohedral map generation 
    //and spell range costs. It may not work in all cases.
	int FLAT_WORLD;
	...
	// definition table
	// If using hexside objects, they will not decay.
	int DECAY;

	// Do we limit the number of mages which can study inside of certain
	// buildings.
	// If using the Earthsea mod, it is assumed you will set this to zero.
	...
	// This has no affect if hexside terrain is enabled.
    ...
    //Note this is currently incompatible with the real_experience gamedef.
	int TACTICS_NEEDS_WAR;
	
	// Whether allies should aid you if you're set to noaid
	int ALLIES_NOAID;

	// Whether hexside terrain objects are enabled
	int HEXSIDE_TERRAIN;
	
	// Whether the Arcadia magic system should be used
	int ARCADIA_MAGIC;
	
	//Whether the Earthsea victory system should be used. This is not fully gamedef'd yet.
	int EARTHSEA_VICTORY;
	
	//Whether experience and study are seperate entities. 
    //Note this is currently incompatible with the tactics_needs_war gamedef and will
